...
def init_db():
    ...
    cursor.execute('''
        ...
        ON consumption(snapshot_date)
    ''')
    
    cursor.execute('''
        CREATE INDEX IF NOT EXISTS idx_rates_dates 
        ON rates(effective_from, effective_to)
    ''')
    
    cursor.execute('''
        CREATE INDEX IF NOT EXISTS idx_billing_period 
        ON billing(billing_period_start, billing_period_end)
    ''')
